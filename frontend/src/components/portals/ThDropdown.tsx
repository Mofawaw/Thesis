...
const ThDropdownPortal: React.FC<ThDropdownPortalProps> = ({ width, height, thColor, children, triggerRef }) => {
  ...
  return ReactDOM.createPortal(
    <>
      <div className="fixed top-0 left-0 w-full h-full z-30 animate-th-fade-in"></div>
      <div
...
const ThDropdown: React.FC<ThDropdownProps> = ({ width, height, thColor, children, trigger }) => {
  ...
  return (
    <>
      ...
      {isOpen &&
        <ThDropdownPortal width={width} height={height} thColor={thColor} triggerRef={triggerRef}>
          {children}
        </ThDropdownPortal>
      }
    </>
